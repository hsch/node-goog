...
node.goog.prototype.initialiseTestingFramework_ = function() {
  ...
  if (specificTest && node.goog.path_.existsSync(
      node.goog.closureUtils_.getPath(process.cwd(), specificTest))) {
    specificTest = null;
  }
...
node.goog.prototype.initialiseTestingFramework2_ = function() {
  if (goog.testing && goog.testing.TestCase) {
    var tc = goog.testing.TestCase.prototype;
    tc['countNumFilesLoaded_'] =
      function() { return 1;  };
    tc.log = function(msg) {
  ...
  process.on('uncaughtException', function(ex) {
    if (ex.message === 'AsyncTestCase.ControlBreakingException') { return; }
    throw ex;
  });
 ...
 * @return {goog.testing.stacktrace.Frame}
*/
node.goog.prototype.parseStackFrameLine_ = function(line) {
 ...
 * @private
 */
node.goog.prototype.stackFramesToString_ = function(frames) {
  var canonical = [];
  var printing = false;
...
node.goog.prototype.onTestComplete_ = function() {
  console.log(node.goog.prototype.colorizeReport(
      this.testCase.getReport(true)));
};
 ...
 */
node.goog.prototype.colorizeReport = function(report) {
  ...
  var underline = node.goog.padString_('', titleLen, '-');
 ...
 * @private
 */
node.goog.padString_ = function(str, length, ch) {
