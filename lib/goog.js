 ...
 * @type {extern_fs}
 */
NodeGoog.fs_ = require('fs');

/**
 * @private
 * @type {extern_path}
 */
NodeGoog.path_ = require('path');
...
NodeGoog.prototype.init = function(opts) {  
  ...
  this.opts_ = opts || require('../bin/utils').closureUtils.readSettingObject();    
...
NodeGoog.prototype.loadCurrentScriptDeps_ = function() {
  ...
  if (file.indexOf('compile.js') < 0 && 
      NodeGoog.path_.existsSync(depsPath + 'deps.js')) {    
    this.loadScript_(depsPath, 'deps.js' );  
  }
};  
...
NodeGoog.prototype.loadAdditionalDeps_ = function() {
  if (!this.opts_.additionalDeps) { return; }
  
  for (var i = 0, len = this.opts_.additionalDeps.length; i < len; i++) {
    var fileName = this.opts_.additionalDeps[i];            
