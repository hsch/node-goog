...
node.tls.createSecurePair = function(credentials, isServer, requestCert, rejectUnauthorized) {
  return node.tls.core_.createSecurePair.apply(node.tls.core_, arguments);
};
...
node.tls.createServer = function(options, listener) {
  return node.tls.core_.createServer.apply(node.tls.core_, arguments);
};
...
node.tls.connect = function(port, options, cb) {
  return node.tls.core_.connect.apply(node.tls.core_, arguments);
};
