...
node.http.createServer = function(requestListener) {
  return node.http.core_.createServer.apply(node.http.core_, arguments);
};
...
node.http.getAgent = function(host, port) {
  return node.http.core_.getAgent.apply(node.http.core_, arguments);
};
...
node.http.request = function(options, cb) {
  return node.http.core_.request.apply(node.http.core_, arguments);
};
...
node.http.get = function(options, cb) {
  return node.http.core_.get.apply(node.http.core_, arguments);
};
...
node.http.createClient = function(port, host) {
  return node.http.core_.createClient.apply(node.http.core_, arguments);
};
...
node.http.cat = function(url, encoding_, headers_) {
  return node.http.core_.cat.apply(node.http.core_, arguments);
};
