 ...
 * </pre>
 * @param {...string} var_args A list of path fragments to resolve.
 */
node.path.resolve = function(var_args) {
  return node.path.core_.resolve.apply(node.path.core_, arguments);
};
...
node.path.normalize = function(path) {
  return node.path.core_.normalize.apply(node.path.core_, arguments);
};
...
node.path.join = function() {
  return node.path.core_.join.apply(node.path.core_, arguments);
};
...
node.path.dirname = function(path) {
  return node.path.core_.dirname.apply(node.path.core_, arguments);
};
...
node.path.basename = function(path, ext) {
  return node.path.core_.basename.apply(node.path.core_, arguments);
};
...
node.path.extname = function(path) {
  return node.path.core_.extname.apply(node.path.core_, arguments);
};
