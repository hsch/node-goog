 ...
 * @param {Array.<nclosure.gennode.type>=} requireNamespaces The namespaces
 ...
 */
nclosure.gennode.clazz.prototype.createConstructor =
    function(desc, args, priv) {
 ...
 */
nclosure.gennode.clazz.prototype.addAttr =
    function(type, name, desc, isStatic) {
  var jsdoc = [];
  if (desc) jsdoc.push(desc);
...
nclosure.gennode.clazz.prototype.addFunct =
  ...
  funct += ') {\n  return ' + this.clazzName_ + '.core_.' + name + '.apply(' +
      this.clazzName_ + '.core_, arguments);\n};';
...
nclosure.gennode.clazz.prototype.toString = function() {
  ...
  if (this.overview_) {
    buffer.push(nclosure.gennode.clazz.buildJSDoc_(
        ['@fileOverview ' + this.overview_]));
    buffer.push('');
  }
