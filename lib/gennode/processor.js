 ...
 *    namespace overview.
 */
nclosure.gennode.processor = function(name, isNamespace, obj, docs, typedata, 
    coreRequires, overrideOverview) {
  ...
  for (var i in this.obj_) {
    ...
    if (isObject) {
      ...
      try {
        ...
        // console.error('Failed to construct: ' + className +
        //  ' e: ' + e.message);
      }
...
nclosure.gennode.processor.prototype.evalFunctionArgs_ = function(name, val) {
  var args = this.evalFunctionArgsFromFunctDef_(name, val);
  if (args.length > 0) { return args; }
  // Now we look at the typedata file
  // and read any implicitly declared args to this function
  // (like http.Server#listen, fs.resolve#var_args)
  return this.evalFunctionArgsFromTypeData_(name);
};


/**
 * Attempts to work out the args of the specified function by parsing
 *    the actual function code.
 * @private
 * @param {string} name The name of the function.
 * @return {Array.<nclosure.gennode.type>} The type of this function args.
 */
nclosure.gennode.processor.prototype.evalFunctionArgsFromTypeData_ =
    function(name) {
  var key = 'node.' + this.name_ + '#' + name + ':';
  var args = [];
  for (var i in this.typedata_) {
    if (i.indexOf(key) !== 0) { continue; }
    var varname = i.substring(key.length);
    var type = this.typedata_[i];
    ...
    args.push(new nclosure.gennode.type(type, varname, desc));
  }
  return args;
};
 ...
 * @return {Array.<nclosure.gennode.type>} The type of this function args.
 */
nclosure.gennode.processor.prototype.evalFunctionArgsFromFunctDef_ =
    function(name, val) {
 ...
 */
nclosure.gennode.processor.prototype.evalFunctionReturnType_ =
    function(name, val) {
...
/**
 * If no type data is found we try to 'guess' here.
 * @private
 * @param {string?} functionName The name of the funciton.
 * @param {string} argName The name of the function argument or 'returns' for
 *    the returns type.
