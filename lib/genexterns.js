...
node.goog.genexterns = function() {
  ...
  if (!this.nodeDir_)
    throw new Error('To run the genextens.js script you must declare ' +
        ...
        'in the bin directory of node-goog');
...
node.goog.genexterns.prototype.run_ = function() {
  ...
  node.goog.genexterns.fs_.writeFileSync(__dirname + '/node.externs.js',
      this.buffer_.join('\n\n')); // Flush the buffer
};
...
node.goog.genexterns.prototype.initialiseDoneExterns_ = function() {
  var staticExterns = node.goog.genexterns.fs_.readFileSync(__dirname +
 ...
 */
node.goog.genexterns.prototype.writeObjectFromMarkdown_ =
    function(markdownFileName, namespace) {
  var contents = node.goog.genexterns.fs_.readFileSync(
      this.nodeDir_ + '/doc/api/' + markdownFileName + '.markdown',
      encoding = 'utf8');
 ...
 * @param {string} o The instance of the object
 */
 ...
 * @param {string} prefix The current running prefix (can be
 *        cascading namespaces)
 * @param {object} obj An instance of the object to inspect for properties
 */
 ...
 *        cascading namespaces)
 * @param {string} prop The property to document
 * @param {object} val The value of the current property, used for type
 *    detection
 */
 ...
 *    detection
 */
node.goog.genexterns.prototype.writePropImpl_ =
